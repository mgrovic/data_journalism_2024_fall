...
**A1.**
https://datawrapper.dwcdn.net/TMZAo/1/
The story is that after the Covid-19 pandemic, election day voting is almost where is left off (2018 between 2022), and then pose it in a way to see how the 2024 election will compare. 

```{r}
votes_by_type <- read_csv("data/md_votes_by_type.csv") |>
  clean_names() |>
  mutate(
    perct_early = (early / total) * 100,
    perct_elcday = (election_day / total) * 100,
    perct_mail = (mail / total) * 100
  )
write_csv(votes_by_type, "md_votes_by_type.csv")
...
**A2.** 
https://datawrapper.dwcdn.net/0TvOo/1/

The data set I chose was 18-24 year olds early voting turnout. I assmued that as all young adults do, they would wait till the last minute to vote, and as the days went on, the number of "early" voters would increase. This is exactly what the graph indicated. 

```{r}
md_voting_18_to_24 <- read_csv("data/ev_totals_18_24.csv") |> 
  clean_names() |>
  mutate(
    total_votes_per_day = allegany + anne_arundel + baltimore_city + baltimore_county + calvert + caroline + carroll + cecil + charles + dorchester + frederick + garrett + harford + howard + kent + montgomery + prince_georges + queen_annes + saint_marys + somerset + talbot + washington + wicomico + worcester)
  
md_voting_18_to_24 <- md_voting_18_to_24 |>
  adorn_totals("row") |>
  as.data.frame()

view(md_voting_18_to_24)

write_csv(md_voting_18_to_24,"ev_totals_18_24.csv")
...
**A3.** 
ActBlue: https://datawrapper.dwcdn.net/B0Kpk/1/ 
WinRed: https://datawrapper.dwcdn.net/Nwh2t/2/
There are a couple big big, key difference between the two maps however they look quite similar. The total amount donated is 10x higher on the ActBlue map than the WinRed map. Although both maps have total donations concentrated in 5 or 6 zipcodes around dc, the WinRed donations are much more spread out. The big story here is why the money is so concentrated in those counties. Obviously it is because the is a larger population, but other zipcodes have a way higher proportional population to the number of votes they have. 


```{r}
winred_zip_total <- read_csv("data/md_winred_zip_totals.csv") 
head(winred_zip_total)

actblue_zip_total <- read_csv("data/md_actblue_zip_totals.csv") 
```
```{r}
...
head(md_zip_population)

```

```{r}
md_zip_actblue_pop <- md_zip_population |>
  left_join(actblue_zip_total, by = "zip") |>
   arrange(desc(donations)) |>
 filter(!is.na(donations)) |>
  select(zip, estimate, donations, total) |>
mutate(donations_per_1000 = (donations / estimate) * 1000) |>
  arrange(desc(total))

write_csv(md_zip_actblue_pop,"md_actblue_zip_totals.csv")
...
```{r}
md_zip_winred_pop <- md_zip_population |>
  left_join(winred_zip_total, by = "zip") |>
   arrange(desc(donations)) |>
 filter(!is.na(donations)) |>
  select(zip, estimate, donations, total) |>
mutate(donations_per_1000 = (donations / estimate) * 1000) |>
arrange(desc(total))
write_csv(md_zip_winred_pop,"md_winred_zip_totals.csv")
