...
There should be 24, as there are 23 counties and Baltimore City. 
```{r}
...
we are attempting to merge two datasets using COUNTY as the key column.
```{r}
...
Prince George's County has both the highest highest 18+ Black population as a percentage of all population 18 and percentage of Democrats compared to other counties. 
...
Howard has the highest pct_asian_18_plus, and is 52% dems. 
```{r}
maryland_population_with_voters_and_demographics |>
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N now? **Answer** chr
