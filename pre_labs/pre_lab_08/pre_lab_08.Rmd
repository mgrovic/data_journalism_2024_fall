...
**Answer**
The first step is saving the code to the dataframe md_voters
The mutate is creating a new column called UNAF_DIFF, which is the difference between Unaffiliated voters from 2022 to 2024. 

The next step is saving the updated dataframe to a new data frame called maryland_top_counties
the next line of code is arranging the UNAF_DIFF column from largest to smallest 
using select on county and UNAF_DIFF means those will be the only columns to apperar. 
Finally head(10), means show the top 10 results. 
```{r}
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**
A bar chart showing the distribution of counties by unaffiliated voters, the labels of the counties are all over the place. 
...
The difference in this code is the bar chart is now organized from least to greatest. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer**
Bar chart is now running left to right instead of down to up, allowing for A  more organized label for the counties. 
```{r}
maryland_top_counties |>
  ggplot() +
  geom_bar(aes(x=reorder(County,UNAF_DIFF), weight=UNAF_DIFF)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer**
the grey undertone is now just white. 
...
**Answer** Different color background, differnet font and the lines on the graph (not the bars but the graph lines) are different. 
...
There is now a title on top, a source has been added, the county label now says net change 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**
The data is organized by date, showing what dates had one number of correlations. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**
I see a line graph, progressivly increasing over time, comparing the date and the total contributions. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**
The graph's trend is the same, however x- axis now identifies more specific dates in intervals of 7 days (weeks). 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**
The text on the X axis is now slanted making it more legible. 
