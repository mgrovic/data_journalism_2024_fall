...
```{r}
create_credentials(GROQ_API_KEY = "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")
```
...
**Task** Run the following code in the gray-colored codeblock below, and tell me what you think the difference between the first 10 names and the second 10 names is, if any. I suppose that the first 10 are more light hearted while the following are a bit more serious and regal. 

**Answer**
I didn't notice much of a difference between the first 10 and last 10 LEMUR names listed. 
```{r}
...
**Answer**
Llama did get it right, however I would make the job_title portion more clear. I was at first confused if job_title referred to the congressman's job or the job being applied for. I would instead call job_title, job_opening or something along those lines. 
```{r}
...
**Answer**
This response does make sense, as it tells you how to properly call in a library to use a pipe operator. With that being said I am not sure this is the correct answer to the problem, as the pipe operator should be able to be used in tidyverse I think...
```{r}
...
**Answer**
Yes this makes sense. This code would certainly help me understand the data, as I would be able to find the most common month and day of registration, a summary of the most common month and day of registration, most common political party and most common trends in the voter addresses. I do not think it makes any false assumptions about the data or anything that seems inncorrect. 

### 6. Finding Patterns in Political Party

Let's find the most common political party.
...
**Answer**
The image depicts a soldier in US army fatigues, wearing a green camouflage military uniform, a helmet with a large number "57" on the side and the Army insignia on the front, goggles, utility-vest, and gloves, while floating in the air above a desert landscape. His arms are outstretched to either side, appearing to hover above a desert expanse below. Next to the soldier is a backpack or duffel bag containing a checkered American flag and an American flag folded and draped over the top of the bag, which appears to be used as a landing pad or a stretcher. A disclaimer is displayed in the foreground of the image, saying "USE OF MILITARY RANK, JOB TITLE, OR PHOTOGRAPHS IN UNIFORM DOES NOT IMPLY ENDORSEMENT BY THE DEPARTMENT OF DEFENSE."

Here are the items described:

1. The soldier is wearing a thick Army uniform.
2. On his head, he is wearing a green military uniform, with a large number fifty-seven and Army insignia.
3. He is also wearing goggles.
4. The soldier is wearing a utility vest.
5. He is wearing gloves.
6. Next to the soldier is a backpack or duffle bag.
7. Inside the backpack, the American flag is folded and draped over the top.
8. A checkered American flag is at the front of the bag.

The LLM did a good job describing what was going on in the image, however lacked non-surface level analysis. It also identified a 57, but I was not able to see the 57 anywhere on the helmet. It also called the hand reaching out to the soldier a duffle bag, incorrectly identifying it. 
...
**Answer** 
The results of the .extract column make sense for the most part, as the majority of the outputs are indeed "merchandises that can be won," some how ever may have confused the LLM, like maga poll and fight fund program. These are things that are most likely not a "merchandise that could be won", but have confused the LLM.
