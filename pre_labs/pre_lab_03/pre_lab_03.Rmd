...
**Answer**
1,640 rows and I think the problem is R encountered some issues interpreting some columns
...
**Answer**
Mail is DBL or a number, precinct is CHR or a character and district is DBL or a number.
...
**Answer**:
There are 402345 NAs for mail, the large number of NA values may because most people didn't mail-in vote. 
```{r}
...
There are 12 registrations from January 1900, which seems extremely unlikely, so it may just be a place holder. 
```{r}
yadkin_voters |> 
...
```{r} 
yadkin_voters |>
  filter(floor_date(registr_dt,"month")==mdy("01-01-1900"))
...
Duplicate is very interesting, as how can their be two different voters in the same county? What do they do about this> 
...
**Answer**
Wrong Type Of Data, missing data and gaps in data
```{r}
conowingo
```
...
Which means that there is an character where there should be a number 
```{r}
# cleaning function
...
All zip codes are now in standard form. 
