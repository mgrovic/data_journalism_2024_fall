...
"W. MILLAR & CO" appears with multiple variations:
"W. MILLAR & CO"
"W. MILLAR & CO."
"W. Millar & Co."
"W.Millar & Company Catering"
```{r}
...
Captial Grill and the Salt Line. 
...
Standardized all W. Millar & CO.'s to Uppercase, Standardized walmart to WALMART and flipped Hilton Washington to Washington Hilton. 
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by city_clean and city to examine cities that start with A. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer** No it seems to be quite similar. 
...
Clustered_DC_FOOD <- read_csv("data/dc-food-clustered.csv")


head(Clustered_DC_FOOD)

```
```{r}
Clustered_DC_FOOD |>
  group_by(recipient_name, recipient_name_orig) |>
  summarise(
    count=n()
  ) |>
  arrange(recipient_name)
