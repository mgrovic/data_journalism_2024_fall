...
The graphic shows every word mentioned in the texts, without multiples
...
Yes this new graphic is way more interesting, as it gets rid of the short, meaningless words and shows the frequency of the word and what percent they occur. 
...
the code shows ther unique words by year and the frequency that they occur and the percentage that they occur in. 
...
**Task** Run the following code and describe the results. **Answer**
samething as before, but instead of single words, it is two word phrases. 
```{r}
releases |>
  filter(year(date) == 2022) |>
...
**Task** Run the following code and describe the results. **Answer**
the two word phrases from 2022 are quite similar to those of 2024 as both data sets top two occuring phrases are health care and amount included or awarded
...
the first one will be more useful as it shows an array of words insteaf of the same word over and over again. 
...
definitely the first one will be more useful, as it groups fund-funding-funds all into one word as the second data set does not. 
...
words like significant are marked positive, but may actually have a negative connotation, as they are more to describe something. if something is a "significant risk" it could have a negative sentiment. 
