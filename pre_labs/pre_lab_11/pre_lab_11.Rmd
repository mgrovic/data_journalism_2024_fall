...
**Answer**
The County column is called Jurisdiction, the columns have thier party listed next to them (ex. Kamala D. Harris and Tim Walz (Democratic))
...
The numbers refer to the column order, so harris = 2, trump = 3, etc
...
**Answer**
Because <li> tags are typically nested within <ul>  the presence of numerous <li> tags suggests that there are  <ul> tags in the HTML data
```{r}
...
The first set contains items like Representatives and Visitors, while the second has items such as Agriculture and Appropriations
...
All the items from the first code are now isolated and cleaned, with the second column containing the URLs
