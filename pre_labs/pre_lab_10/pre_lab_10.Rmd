...
This data shows Precinct ID,  Precinct Name, congressional district, Legislation, Council,  # of School’s, Shape Length, Shape Area and Shape     
...
It contains  the same categories as listed above. In the shape column is actually shows some of the shapes of the precinct  
...
The output is a map of PG County divided into precincts, on a longitudinal map 
...
**Task** Run the following code. Describe the output in the space below. **Answer**
The output is a bunch of FIPS codes for States, Counties, etc and different areas and shapes. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** 
Map of Maryland divided by county. It does seem to lack the water features if Maryland. 
...
**Task** Run the following code. Describe the output in the space below. **Answer**
Now the map has isolated PG county from the maryland map, and then put the pricincts on top. 
...
**Task** Run the following code. Describe the output in the space below. **Answer**
This Data shows PG county pricincts and their voting break downs. 
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
The same map as above is now portraying the total eligible voters by precinct in PG County.
```{r}
ggplot() +
...
**Task** Run the following code. Describe the output in the space below. **Answer**
This shows voters by County in Maryland and which way their constituents voted. 
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
This shows the estimated population in Maryland counties. 
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
The output is the Counties in Maryland, their estimated population and the shape of that county. 
...
**Task** Run the following code. Describe the output in the space below. **Answer**
 This is the same data as before with Name now called County and the Maryland part of the name now removed. 
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
Now this shows the Counties, their population, and the way that population voted. 
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
This cleaned the names of two counties in Maryland
```{r}
md_county_population <- md_county_population |>
...
**Task** Run the following code. Describe the output in the space below. **Answer**
This joined the two data sets and also made a new column that shows the number of voters per 10 thousand people. 
```{r}
md_voters_per_10k <- md_county_population |>
  left_join(voters_by_county) |>
...
**Task** Run the following code. Describe the output in the space below. **Answer**
This is a map with the maryland counties portraying how many voters there were per 10,000 people in the county. 
```{r}
ggplot() +
...
**Task** Run the following code. Describe the output in the space below. **Answer**
Same map, with the same data, but now the colors are changed so the differences are more visable. 
```{r}
ggplot() +
  geom_sf(data=md_voters_per_10k, aes(fill=voters_per_10k)) +
  theme_minimal() +
...
I didn't see a differnce in this map. 
```{r}
ggplot() +
  geom_sf(data=md_voters_per_10k, aes(fill=voters_per_10k)) +
  theme_minimal() +
...
This is the same map of Maryland counties, but the data now shows unafilated voters per 10 thousand people. 

Some of the regional patterns i see are that the most unaffilated voters are in the middle of the state. 
